...
def get_model(input_shape):
    ...
    '''
    model = Sequential()
    model.add(Dense(1, input_dim=input_shape, activation='linear'))
    model.compile(loss='mae',
                  optimizer='adam',
                  )
    '''
...
if impersonal:
    data = pd.read_pickle('pkl/datasets/dataset_lags{0}_period{1}_gran1h.pkl'.format(time_lags, period))
else:
